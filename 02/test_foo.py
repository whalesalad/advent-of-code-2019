...
def test_example_a():
    ...
    assert ','.join([str(x) for x in c.run()]) == '3500,9,10,70,2,3,11,0,99,30,40,50'


def test_example_b():
    c = Computer('1,1,1,4,99,5,6,0,99')
    assert ','.join([str(x) for x in c.run()]) == '30,1,1,4,2,5,6,0,99'
...
def test_part_one():
    ...
    print(f'VALUE IS: {c.run()[0]}')

    assert c.run()[0] == 4023471
